// coLaB LiveSync v7.0 — Hardcoded M4L-only sync engine
// Polls ALL params via LiveAPI, sends deltas via UDP, applies incoming.
// No Node.js, no web-bridge, no scanning. One file, two machines.
//
// Patcher: [live.thisdevice]→[init]→[js colab_livesync.js]
//          [metro 100]→[poll]→[js]
//          [js]→[udpsend PEER 8001]
//          [udpreceive 8001]→[js]
...
var mixSnap = [];       // [{vol,pan,mute,solo,arm,color,name}] per track
var devSnap = {};       // { 'T:D': [val, val, ...] }
var masterSnap = {};    // { 'D': [val, val, ...] }
var foldSnap = {};      // { trackIdx: bool }
var clipSnap = {};      // { 'T:C': bool (has_clip) }
var noteHash = {};      // { 'T:C': hash }
var tempoSnap = 0;
var echoLock = {};
...
function init() {
  post("coLaB LiveSync v7.0 init...\n");

  // Read initial mixer state
  mixSnap = [];
  for (var i = 0; i < TRACK_COUNT; i++) {
    mixSnap.push(readMixer(i));
  }

  // Read initial device params
  ...
  outlet(0, "host", PEER);
  outlet(0, "port", 8001);
  post("LiveSync ready. " + TRACK_COUNT + " tracks, peer=" + PEER + "\n");
}
...
function readClipNotes(trackIdx, clipIdx) {
  try {
    ...
    // notes comes as flat array: [pitch, start, dur, vel, mute, ...]
    var result = [];
  ...
  for (var c = 0; c < SCENE_COUNT; c++) {
    ...
    try {
      ...
      if (old !== undefined && has && !old) {
        // New clip — read its info and notes
        clipSnap[key] = true;
...
function pollNotes() {
  // Check one clip per poll for note changes
...
function anything() {
  ...
  // Handle incoming UDP (via prepend or raw)
...
post("coLaB LiveSync v7.0 loaded. Send init to start.\n");
